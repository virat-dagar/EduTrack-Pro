...
class AssignmentService:
    ...
    def list_assignments(
        ...
        if current_user.role == ROLE_STUDENT:
            ...
            query = query.filter(Assignment.is_active.is_(True), Assignment.is_published.is_(True))
            query = query.filter(
                or_(
    ...
    def create_assignment(db: Session, payload: AssignmentCreate, teacher_id: int) -> Assignment:
        ...
        data["total_marks"] = AssignmentService._total_marks(payload.total_marks, payload.questions)
    ...
    def update_assignment(db: Session, assignment_id: int, payload: AssignmentUpdate) -> Assignment:
        ...
        questions = payload.questions if "questions" in payload.model_fields_set else None
        ...
        if data.get("classroom_id") is not None:
            if db.query(Classroom).filter(Classroom.id == data["classroom_id"]).first() is None:
                raise NotFoundError("Classroom not found.")
        ...
        if questions is not None:
            ...
            assignment.total_marks = AssignmentService._total_marks(assignment.total_marks, questions)
        db.commit()
        db.refresh(assignment)
        return assignment
    ...
    def submission_summary(db: Session, assignment_id: int) -> dict[str, object]:
        ...
        if assignment.classroom_id:
            assigned_count = (
                db.query(Student)
                .filter(Student.classroom_id == assignment.classroom_id, Student.is_active.is_(True))
                .count()
            )
        else:
            assigned_count = 0
        ...
        return {
            ...
            "reviewed": len([item for item in assignment.submissions if item.reviewed_by is not None]),
        }
    ...
    def enrich(assignment: Assignment) -> dict[str, object]:
        ...
        return {
            ...
            "classroom": assignment.classroom.classroom_name if assignment.classroom else None,
            "questions": [
                {
                    "id": question.id,
    ...
    @staticmethod
    def enrich_for_user(db: Session, assignment: Assignment, current_user: User) -> dict[str, object]:
        ...
        data.update(
            {
                ...
                "submission_file": submission.submitted_file or submission.attachment_path,
    ...
    def _total_marks(total_marks: float | None, questions: list[object]) -> float:
        ...
        if questions:
            return float(
                sum(
                    question["max_marks"] if isinstance(question, dict) else question.max_marks
